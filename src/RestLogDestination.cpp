...
RestLogDestination::RestLogDestination(log_level level, std::string hub_url) : LogDestination(level){
	this->hub_url = hub_url;
	this->stop = false;
}

bool RestLogDestination::Init(){
	this->stop = false;
	std::thread *t = new std::thread(&ydleMaster::RestLogDestination::run, this);
	return true;
}

void RestLogDestination::run(){
	while(!this->stop){
...
void RestLogDestination::Sending(Json::Value & message){
	CURL *curl;
	CURLcode res;
	char *error_buf;
	...
	if(curl) {
		...
		curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, &ydleMaster::RestLogDestination::responseCallback);
...
size_t RestLogDestination::responseCallback( char *response, size_t size, size_t nmemb, void *userdata){
	size_t realsize = size * nmemb;
	// Rajouter ici un traitement si erreur
	return realsize;
}

void RestLogDestination::Sending(log_message & message){
	CURL *curl;
	CURLcode res;
	char *error_buf;
	std::stringstream data;
...
void RestLogDestination::Write(log_level level, const string &log_line){
